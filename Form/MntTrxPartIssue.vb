...
Public Class MntTrxPartIssue
    ...
    Public Sub New(_userId As Integer, Optional _partTrxId As Integer = 0, Optional _isFloat As Boolean = False)
        ...
        Else
            ...
            Else
                ...
                btnReturnAllItem.Visible = False
            End If
    ...
    Private Sub btnAdd_Click(sender As Object, e As EventArgs) Handles btnAdd.Click
        Try
            ...
            If String.IsNullOrWhiteSpace(txtQty.Text) OrElse CInt(txtQty.Text.Trim) = 0 Then
                MessageBox.Show("Please input quantity to issue.", "", MessageBoxButtons.OK, MessageBoxIcon.Error)
                txtQty.Focus()
                Exit Sub
            End If
    ...
    Private Sub MntTrxPartIssue_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Dim colRemainingQty As DataGridViewTextBoxColumn = New DataGridViewTextBoxColumn()
        colRemainingQty.Name = "ColRemainingQty"
        colRemainingQty.DataPropertyName = "RemainingQty"
        colRemainingQty.HeaderText = "Remaining"
        colRemainingQty.Width = 80
        colRemainingQty.DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
        colRemainingQty.SortMode = DataGridViewColumnSortMode.Automatic
        dgvPartDetail.Columns.Insert(5, colRemainingQty)
    ...
    Private Sub btnReturnAllItem_Click(sender As Object, e As EventArgs) Handles btnReturnAllItem.Click
        Try
                ...
                If MessageBox.Show(question, "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                    For Each row As DataRowView In Me.bsTrxPartDetailFloat
                        If row("RemainingQty") > 0 Then
                            ...
                            Dim updDtlQry = "UPDATE dbo.MntTransactionPartDetailFloat SET ConsumedQty = ConsumedQty + @RemainingQty, RemainingQty = 0 WHERE PartTrxDetailId = @PartTrxDetailId"
        ...
        Try
            If chkFloat.Checked Then
                For i As Integer = 0 To dgvPartDetail.Rows.Count - 1
                    Dim issuedQty As Integer = dgvPartDetail.Rows(i).Cells("ColIssuedQty").Value
    ...
    Private Sub btnReturnItem_Click(sender As Object, e As EventArgs) Handles btnReturnItem.Click
        Try
            If dgvPartDetail.Rows.Count > 0 AndAlso dgvPartDetail.SelectedRows.Count > 0 Then
                ...
                Dim issuedQty As Integer = CType(Me.bsTrxPartDetailFloat.Current, DataRowView).Item("IssuedQty")
                Dim consumedQty As Integer = CType(Me.bsTrxPartDetailFloat.Current, DataRowView).Item("ConsumedQty")
                Dim remainingQty As Integer = CType(Me.bsTrxPartDetailFloat.Current, DataRowView).Item("RemainingQty")
                Dim partId As Integer = CType(Me.bsTrxPartDetailFloat.Current, DataRowView).Item("PartId")

                If issuedQty <> consumedQty Then
                    Dim prmLog(4) As SqlParameter
                    prmLog(0) = New SqlParameter("@PartTrxDetailId", SqlDbType.Int)
                    ...
                    prmDetailFloat(1) = New SqlParameter("@RemainingQty", SqlDbType.Int)
                    prmDetailFloat(1).Value = remainingQty
                    Dim updDtlQry = "UPDATE dbo.MntTransactionPartDetailFloat SET ConsumedQty = ConsumedQty + @RemainingQty, RemainingQty = 0 WHERE PartTrxDetailId = @PartTrxDetailId"
                    dbMethod.ExecuteNonQuery(updDtlQry, CommandType.Text, prmDetailFloat)
