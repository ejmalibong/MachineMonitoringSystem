...
Public Class FacTrxPartIssue
    ...
    Public Sub New(Optional _userId As Integer = 0, Optional _partTrxId As Integer = 0)
        ...
        If partTrxId = 0 Then
            ...
            dtTrxPartDetail = CreateFacTransactionPartDetail()
            'dtTrxPartDetailFloat = CreateFacTransactionPartDetailFloat()
        ...
        Else
            ...
            dtTrxPartHeader = dbMethod.FillDataTable("RdFacTransactionPartHeaderByPartTrxId", CommandType.StoredProcedure, prmHeader)

            Dim prmDetail(0) As SqlParameter
            prmDetail(0) = New SqlParameter("PartTrxId", SqlDbType.Int)
            prmDetail(0).Value = partTrxId

            dtTrxPartDetail = dbMethod.FillDataTable("RdFacTransactionPartDetailByPartTrxId", CommandType.StoredProcedure, prmDetail)
        ...
        prmPart(0).Value = 1
        dtSparePart = dbMethod.FillDataTable("SELECT PartId, TRIM(PartNo) AS PartNo, TRIM(PartName) AS PartName FROM dbo.FacSparePart WHERE IsActive = @IsActive AND ActualStock > 0", CommandType.Text, prmPart)
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        Try
            ...
            If chkFloat.Checked Then
                ...
                'dbMethod.ExecuteNonQuery("InsFacTransactionPartHeaderFloat", CommandType.StoredProcedure, prmPrHeaderFloat)
            ...
            Else
                ...
                dbMethod.ExecuteNonQuery("InsFacTransactionPartHeader", CommandType.StoredProcedure, prmPrHeader)
                ...
                For Each row As DataGridViewRow In dgvPartDetail.Rows
                    ...
                    prmIss(1).Value = row.Cells("ColIssuedQty").Value

                    dbMethod.ExecuteNonQuery("UpdFacSparePartIss", CommandType.StoredProcedure, prmIss)
                Next
            End If
        ...
        Try
            If cmbPart.SelectedValue <> 0 Then
                ...
                If cmbPartSelection.SelectedValue = 1 Then
                    ...
                    Using rdr As IDataReader = dbMethod.ExecuteReader("RdFacSparePart", CommandType.StoredProcedure, prmPartNo)
                        While rdr.Read
    ...
    Private Sub cmbPartSelection_SelectedValueChanged(sender As Object, e As EventArgs) Handles cmbPartSelection.SelectedValueChanged
        Try
            ...
            If cmbPartSelection.SelectedValue = 1 Then
                cmbPart.DisplayMember = "Specs"
                cmbPart.ValueMember = "PartId"

                dbMethod.FillCmbWithCaption("SELECT PartId, TRIM(PartName) + ' ' + TRIM(PartNo) AS PartName FROM dbo.FacSparePart WHERE ActualStock > 0 AND IsActive = 1",
            ...
            Else
                ...
                dbMethod.FillCmbWithCaption("SELECT PartId, TRIM(PartNo) AS PartNo FROM dbo.FacSparePart WHERE ActualStock > 0 AND IsActive = 1",
    ...
    Private Function CreateFacTransactionPartDetail(Optional partTrxId As Integer = 0) As DataTable
        Dim dtFacTrxPartDetail As New DataTable
        Dim con As New SqlConnection(dbConnection.GetConnectionString)

        Try
            Dim query As String = String.Empty

            query = "SELECT PartTrxDetailId, PartTrxId, SeqId, CreatedBy, CreatedDate, UserId, PartId, Qty, ModifiedBy, ModifiedDate FROM dbo.FacTransactionPartDetail"
            ...
            dtFacTrxPartDetail.Columns.Add(colModifiedBy)
            ...
            dtFacTrxPartDetail.Columns.Add(colModifiedDate)
        Catch ex As Exception
            MessageBox.Show(dbMain.SetExceptionMessage(ex), "", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try

        Return dtFacTrxPartDetail
    End Function

    'Private Function CreateFacTransactionPartDetailFloat(Optional partTrxId As Integer = 0) As DataTable
    '    Dim dtFacTrxPartDetailFloat As New DataTable
    '    Dim con As New SqlConnection(dbConnection.GetConnectionString)

    '    Try
    '        Dim query As String = String.Empty

    '        query = "SELECT PartTrxDetailId, PartTrxId, SeqId, CreatedBy, CreatedDate, IssuedTo, PartId, IssuedQty, ConsumedQty, RemainingQty, ModifiedBy, ModifiedDate FROM dbo.FacTransactionPartDetailFloat"
    ...
    '        Dim colPartTrxDetailId As DataColumn = New DataColumn("PartTrxDetailId")
    '        colPartTrxDetailId.DataType = System.Type.GetType("System.Int32")
    '        dtFacTrxPartDetailFloat.Columns.Add(colPartTrxDetailId)

    '        Dim colPartTrxId As DataColumn = New DataColumn("PartTrxId")
    '        colPartTrxId.DataType = System.Type.GetType("System.Int32")
    '        dtFacTrxPartDetailFloat.Columns.Add(colPartTrxId)

    '        Dim colSeqId As DataColumn = New DataColumn("SeqId")
    '        colSeqId.DataType = System.Type.GetType("System.Int32")
    '        dtFacTrxPartDetailFloat.Columns.Add(colSeqId)

    '        Dim colCreatedBy As DataColumn = New DataColumn("CreatedBy")
    '        colCreatedBy.DataType = System.Type.GetType("System.Int32")
    '        dtFacTrxPartDetailFloat.Columns.Add(colCreatedBy)

    '        Dim colCreateDate As DataColumn = New DataColumn("CreatedDate")
    '        colCreateDate.DataType = System.Type.GetType("System.DateTime")
    '        dtFacTrxPartDetailFloat.Columns.Add(colCreateDate)

    '        Dim colIssuedTo As DataColumn = New DataColumn("IssuedTo")
    '        colIssuedTo.DataType = System.Type.GetType("System.Int32")
    '        dtFacTrxPartDetailFloat.Columns.Add(colIssuedTo)

    '        Dim colPartId As DataColumn = New DataColumn("PartId")
    '        colPartId.DataType = System.Type.GetType("System.Int32")
    '        dtFacTrxPartDetailFloat.Columns.Add(colPartId)

    '        Dim colIssuedQty As DataColumn = New DataColumn("IssuedQty")
    '        colIssuedQty.DataType = System.Type.GetType("System.Int32")
    '        dtFacTrxPartDetailFloat.Columns.Add(colIssuedQty)

    '        Dim colConsumed As DataColumn = New DataColumn("ConsumedQty")
    '        colConsumed.DataType = System.Type.GetType("System.Int32")
    '        dtFacTrxPartDetailFloat.Columns.Add(colConsumed)

    '        Dim colRemaining As DataColumn = New DataColumn("RemainingQty")
    '        colRemaining.DataType = System.Type.GetType("System.Int32")
    '        dtFacTrxPartDetailFloat.Columns.Add(colRemaining)

    '        Dim colModifiedBy As DataColumn = New DataColumn("ModifiedBy")
    '        colModifiedBy.DataType = System.Type.GetType("System.Int32")
    '        colModifiedBy.AllowDBNull = True
    '        dtFacTrxPartDetailFloat.Columns.Add(colModifiedBy)

    '        Dim colModifiedDate As DataColumn = New DataColumn("ModifiedDate")
    '        colModifiedDate.DataType = System.Type.GetType("System.DateTime")
    '        colModifiedDate.AllowDBNull = True
    '        dtFacTrxPartDetailFloat.Columns.Add(colModifiedDate)
    '    Catch ex As Exception
    '        MessageBox.Show(dbMain.SetExceptionMessage(ex), "", MessageBoxButtons.OK, MessageBoxIcon.Error)
    '    End Try

    '    Return dtFacTrxPartDetailFloat
    'End Function
    ...
    Private Sub FacTrxPartIssue_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        LoadPartSelection()
        ...
        If partTrxId = 0 Then
            ...
            colIssuedQty.DataPropertyName = "Qty"
    ...
    Private Sub FacTrxPartIssue_Shown(sender As Object, e As EventArgs) Handles MyBase.Shown
        Try
            If partTrxId <> 0 Then
