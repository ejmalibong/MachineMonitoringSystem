...
Public Class FacSparePartLog
    ...
    Private Sub btnExport_Click(sender As Object, e As EventArgs) Handles btnExport.Click
        Try
            ...
            Dim query As String = "SELECT TrxId, TrxDate, TransactionCode, UserName, PartNo, PartName, Qty, ReferenceNo, ParticularName, AreaName, MachineStatusName, MachineSubStatusName, Problem, RootCause, ActionTaken, Remarks FROM dbo.VwFacTransactionPartLogs WHERE "
    ...
    Private Sub btnView_Click(sender As Object, e As EventArgs) Handles btnView.Click
        Try
            If Me.dgvList.Rows.Count > 0 Then
                ...
                ' (temporary) use partTrxId because parts receiving and issuance is not yet integrated to activity report
                ...
                '        Using frmReceive As New FacTrxPartReceive(0, partTrxId)
                '            frmReceive.ShowDialog()
                '        End Using
                '    Else
                '        Using frmIssue As New FacTrxPartIssue(0, partTrxId)
                '            frmIssue.ShowDialog()
                '        End Using
                '    End If
                ...
                '    Dim rdr As IDataReader = dbMethod.ExecuteReader("SELECT Machineid, JigId FROM FacTransactionHeader WHERE TrxId = @TrxId", CommandType.Text, prm)

                '    While rdr.Read
                '        If rdr.Item("MachineId") Is DBNull.Value AndAlso rdr.Item("JigId") Is DBNull.Value Then
                '            Using frmDetail As New FacTrxDetailOth(0, 0, 0, trxId)
                '                frmDetail.ShowDialog()
                '            End Using
                '        End If

                '        If Not rdr.Item("MachineId") Is DBNull.Value Then
                '            Using frmDetail As New FacTrxDetailMch(0, 0, 0, trxId)
    ...
    Private Sub LoadLogs()
        Try
            ...
            If isFilterByTrxDate = True Then
                ...
                dtSparePart = dbMethod.FillDataTable("RdFacTransactionPartLogsTrxDate", CommandType.StoredProcedure, prmPart)
            ...
            ElseIf isFilterByUsername = True Then
                ...
                dtSparePart = dbMethod.FillDataTable("RdFacTransactionPartLogsUserId", CommandType.StoredProcedure, prmPart)
                totalCount = prmPart(2).Value
                totalQty = prmPart(3).Value
            ...
            ElseIf isFilterByPartNo = True Or isFilterByPartName = True Then
                ...
                dtSparePart = dbMethod.FillDataTable("RdFacTransactionPartLogsPartId", CommandType.StoredProcedure, prmPart)
                totalCount = prmPart(2).Value
                totalQty = prmPart(3).Value
            ...
            Else
                ...
                dtSparePart = dbMethod.FillDataTable("RdFacTransactionPartLogs", CommandType.StoredProcedure, prmPart)
                totalCount = prmPart(2).Value
                totalQty = prmPart(3).Value
            End If
    ...
    Private Sub LoadPartName()
        dbMethod.FillCmbWithCaption("SELECT PartId, TRIM(PartName) + ' ' + TRIM(PartNo) AS PartName FROM dbo.FacSparePart", CommandType.Text, "PartId", "PartName", cmbCommon2, "< All >")
    End Sub

    Private Sub LoadPartNo()
        dbMethod.FillCmbWithCaption("SELECT PartId, TRIM(PartNo) AS PartNo FROM dbo.FacSparePart", CommandType.Text, "PartId", "PartNo", cmbCommon2, "< All >")
    End Sub
    ...
    Private Sub FacSparePartLog_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        LoadSearchCriteria()
        LoadSortCriteria()
